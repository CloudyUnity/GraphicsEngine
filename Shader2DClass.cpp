...
bool Shader2DClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	result = ShaderClass::InitializeShader(device, hwnd, "../GraphicsEngine/Simple.vs", "../GraphicsEngine/2D.ps", false, true);
	if (!result)
		return false;
...
bool Shader2DClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, TextureSetClass* textures)
{
	bool result;
