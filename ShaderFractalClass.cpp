...
bool ShaderFractalClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	result = ShaderClass::InitializeShader(device, hwnd, "../GraphicsEngine/Fog.vs", "../GraphicsEngine/Fractal.ps", false, false);
	if (!result)
		return false;
...
bool ShaderFractalClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, TextureSetClass* textures)
{
	bool result;
