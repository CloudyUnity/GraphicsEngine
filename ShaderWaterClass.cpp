...
bool ShaderWaterClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	result = ShaderClass::InitializeShader(device, hwnd, "../GraphicsEngine/Water.vs", "../GraphicsEngine/Water.ps", false, false);
	if (!result)
		return false;
	...
	bool bufferCreationResult =
		...
		TryCreateBuffer(device, m_bufferDesc, m_CBufferWater) &&
...
bool ShaderWaterClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, TextureSetClass* textures)
{
	bool result;
	...
	*reflectPtr = m_CBufferReflect.BufferValues;
	...
	if (!TryMapBuffer(deviceContext, &m_CBufferWater.BufferPtr, &waterPtr))
		return false;
	*waterPtr = m_CBufferWater.BufferValues;
	UnmapFragmentBuffer(deviceContext, 5, &m_CBufferWater.BufferPtr);
